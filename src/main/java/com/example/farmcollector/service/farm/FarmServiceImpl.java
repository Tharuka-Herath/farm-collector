...
import com.example.farmcollector.exception.FarmValidationException;
...
public class FarmServiceImpl implements FarmService {
    ...
    public void validateFarm(FarmDTO farmDTO) {
        validateFarmName(farmDTO.getFarmName());
        validateFarmLocation(farmDTO.getLocation());
        validateFarmArea(farmDTO.getFarmArea());
//        validateWeatherData(farmDTO.getWeatherData());
    }

    public void validateFarmName(String farmName) {
        if (farmName == null || farmName.isEmpty()) {
            throw new FarmValidationException("Farm name is required");
        }
        if (!farmName.matches("^[a-zA-Z\\s]+$")) {
            throw new FarmValidationException("Invalid Farm name. Cannot contain numbers");
        }
    }

    public void validateFarmLocation(String farmLocation) {
        if (farmLocation == null || farmLocation.isEmpty()) {
            throw new FarmValidationException("Farm location is required");
        }
        if (!ALLOWED_LOCATIONS.contains(farmLocation.toUpperCase())) {
            throw new FarmValidationException("Invalid Farm location. Mention the district");
        }
    }


//    public void validateWeatherData(WeatherDataDTO weatherData) {
//        if (weatherData == null) {
//            throw new FarmValidationException("Weather data not found");
//        }
//    }

    public void validateFarmArea(Double farmArea) {
        if (farmArea == null || farmArea <= 0) {
            throw new FarmValidationException("Invalid farm area");
        }
    }
}
